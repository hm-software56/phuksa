...
<div id="print">
    ...
    if(isset($_GET['month_start'])&& isset($_GET['month_end']))
    {
        $service_type=ServiceTicket::find()->where(['id'=>$_GET['type']])->one();
        $ticket_type="";
        if($service_type)
        {
    ...
    <table class="table table-bordered">
        <tr>
            <td colspan="3" align="right"><?=Yii::t('app','ລາຍ​ງານ​ການ​ຂາຍ​ປີ້​ເຂົ້າ​ສວນ​ພຶກ​ສາເດືອນ:').date('m/Y',strtotime($_GET['month_start']))." ຫາ ".date('m/Y',strtotime($_GET['month_end'])).", ".Yii::t('app','ປະ​ເພດ​ປີ້:').$ticket_type ?></td>
        </tr>
        <tr>
            <th><?=Yii::t('app','ເດືອນ')?></th>
            <th><?=Yii::t('app','ລວມ​ຈຳ​ນວນຄົນ​ເຂົ້າ')?></th>
        ...
        <?php
            ...
            while($start <= $end)
            {
                ...
                if(!empty($_GET['type']))
                {
                    $command_amount = $connection->createCommand('SELECT SUM(price*quantity) FROM order_ticket WHERE service_ticket_id='.$_GET['type'].' and month(order_date)="'.date('m-Y', $start).'"')->queryScalar();
                    $command_ps = $connection->createCommand('SELECT SUM(quantity) FROM order_ticket WHERE service_ticket_id='.$_GET['type'].' and month(order_date)="'.date('m-Y', $start).'"')->queryScalar();
                
                }else{
                    $command_amount = $connection->createCommand('SELECT SUM(price*quantity) FROM order_ticket WHERE month(order_date)="'.date('m-Y', $start).'"')->queryScalar();
                    $command_ps = $connection->createCommand('SELECT SUM(quantity) FROM order_ticket WHERE month(order_date)="'.date('m-Y', $start).'"')->queryScalar();
                }
