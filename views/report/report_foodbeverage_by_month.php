...
<div id="print">
	...
	<table class="table table-bordered">
		<tr>
			<td colspan="6" align="right"><?=Yii::t('app','ລາຍ​ງານ​ການ​ຂາຍອາ​ຫານ ​ແລະ​ ເຄື່ອງ​ດື່ມ​ສວນ​ພຶກ​ສາເດືອນ:')?><?=isset($_GET['month_start'])?date('d/Y', strtotime($_GET['month_start'])):date('d/Y')?> - <?=isset($_GET['month_end'])?date('m/Y', strtotime($_GET['month_end'])):date('m/Y')?>, <?=Yii::t('app','ປະ​ເພດ:').$type_name?></td>
		</tr>
		<tr>
			<th><?=Yii::t('app','')?></th>
		...
		while($start <= $end)
		{
		 ...
		 $sale_fbs = $connection->createCommand('SELECT id FROM sale_food_beverage WHERE month(date)="'.date('m', $start).'" and year(date)="'.date('Y', $start).'"')->queryAll();
		 $id_sal=[0];
		...
		?>
		<tr>
			<td  colspan="4" style="background-color:#F3F6F7"><b><?=Yii::t('app','ເ​ດືອນ:').date('m-Y',$start)?></b></td>
		</tr>
		<?php
