...
<div id="print">
	...
	<table class="table table-bordered">
		<tr>
			<td colspan="6" align="right"><?=Yii::t('app','ລາຍ​ງານ​ການສັ່ງ​ຊື້​ສີນ​ຄ້າ​ເຂົ້າ​ສວນ​ພຶກ​ສາເດືອນ:')?><?=isset($_GET['month_start'])?date('m/d/Y', strtotime($_GET['month_start'])):date('m/d/Y')?> - <?=isset($_GET['month_end'])?date('m/d/Y', strtotime($_GET['month_end'])):date('m/d/Y')?>, <?=Yii::t('app','ປະ​ເພດ:').$type_name?></td>
		</tr>
		<tr>
			<th><?=Yii::t('app','')?></th>
		...
		while($start <= $end)
		{
		 ...
		 $sale_fbs = $connection->createCommand('SELECT id FROM product_order WHERE month(order_date)="'.date('m', $start).'" and year(order_date)="'.date('Y', $start).'" and status="Done"')->queryAll();
		 $id_sal=[0];
		...
		?>
		<tr>
			<td  colspan="4" style="background-color:#F3F6F7"><b><?=Yii::t('app','ເດືອນ:').date('m-Y',$start)?></b></td>
		</tr>
		<?php
